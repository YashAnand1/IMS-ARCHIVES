...
var (
	// File paths
	excelFile = "/home/user/Complete/etcd-inventory/etcd.xlsx"
	csvFile   = "/home/user/Complete/etcd-inventory/myetcd.csv"
...
func getSpecificKey(w http.ResponseWriter, r *http.Request) {
	...
	response, err := etcdClient.Get(ctx, etcdKeyData, clientv3.WithSort(clientv3.SortByCreateRevision, clientv3.SortAscend))
	if err != nil {
		fmt.Printf("Could not retrieve data due to %v", err)
	}
	...
	if err != nil {
		fmt.Printf("Could not get revision values due to %v", err)
	}
	w.Header().Set("Content-Type", "text/plain")

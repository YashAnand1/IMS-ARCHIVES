...
func uploadToEtcd() {
	...
	for i = 0; i < len(records[0]); i++ {
		records[0][i] = strings.ToUpper(records[0][i])
	}
...
func getSpecificKeyAnoop(w http.ResponseWriter, r *http.Request) {
	...
	response, err := etcdClient.Get(ctx, etcdKeyData, clientv3.WithPrefix(), clientv3.WithSort(clientv3.SortByCreateRevision, clientv3.SortAscend))
	if err != nil {
		fmt.Printf("%v", err)
	}
...
func deleteSpecificKEy(w http.ResponseWriter, r *http.Request) {
	...
	response, err := etcdClient.Delete(ctx, etcdKeyData, clientv3.WithPrefix(), clientv3.WithSort(clientv3.SortByCreateRevision, clientv3.SortAscend))
	if err != nil {
		fmt.Printf("%v", err)
	} else {
		fmt.Printf("%v", response.kvs)
	}

	w.Header().Set("Content-Type", "text/plain")
	for _, kv := range response.Kvs {
		fmt.Fprintf(w, "Key: %s has been deleted\n", string(kv.Key))
		fmt.Fprintf(w, "----------------------------\n")
	}
	////////////////////
}

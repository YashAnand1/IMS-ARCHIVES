package cmd

import (
	"bytes"
	"fmt"
	"net/http"

	"github.com/spf13/cobra"
)

// deleteCmd represents the delete command
var DELETE = &cobra.Command{
	Use:   "DELETE",
	Short: "Delete keys",
	Long:  `Deletes key-value pairs`,
	Run: func(cmd *cobra.Command, args []string) {
		fmt.Println("delete called")

		// var url string = addressServer + "/delete"
		etcdKey := args[0]
		var url string = addressServer + "/servers/delete" //To change localhost with the conf file

		////////////////////////

		//var url string = addressServer + "/IMS/Anoop"

		var line string = "{" + "\"EtcdKey\"" + ":" + etcdKey + "}"
		fmt.Println(line)
		fmt.Println(url)
		var jsonStr = []byte(line)
		responseBody := bytes.NewBuffer(jsonStr)

		////////////////////////////////////////
		// create a new HTTP client
		client := &http.Client{}
		// create a new DELETE request
		req, err := http.NewRequest("DELETE", url, responseBody)
		if err != nil {
			panic(err)
		}
		// send the request
		resp, err := client.Do(req)
		if err != nil {
			panic(err)
		}
		defer resp.Body.Close()

		if resp.StatusCode == 200 {
			fmt.Printf("Key: %s deleted succesfully\n", line) // Data being deleted but not in DB atm
		}
	},
}

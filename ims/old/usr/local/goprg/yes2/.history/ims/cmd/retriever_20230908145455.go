...
func fetchDataFromAPI() (string, error) { //returns string and error
	...
	response, err := http.Get(addressServer + "/") //Get request sent to the API URL for fetching data
	if err != nil {
		fmt.Printf("%s", err)
	}
...
func fetchDataFromAPIWithKey(key string) (string, error) { //returns string and error
	...
	//url = addressServer + '/' + key
	var url string = fmt.Sprintf("%s/%s", addressServer, key) //Removed dual declaration of url string
	...
	// etcdKey="/IMS/Physical/10.246.40.139/Hostname"
...
// 			etcdKey := fmt.Sprintf("/IMS/%s/%s/%s", serverType, serverIP, header)
// 			etcdValue := value
// 			// fmt.Println(etcdKey)
...
// 		etcdKeyData := fmt.Sprintf("/IMS/%s/%s/data", serverType, serverIP)

...
`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		////////////  anoop ///////////
		if len(args) == 1 { // length f the arg can be >1
			key1 = args[0]                               //args []string - if string of slice args is indexed 0
			numberof_key = len(strings.Split(key1, "/")) //number of key are no of components
		}

		//fmt.Printf("key1:%s", key1)

		if numberof_key == 3 { //turning it into 3 because /servers/Physical/IP/Hostname is to be run without first slash to get key
			...
			if err1 != nil {
				...
				fmt.Println("Key is incorrect or value associated with the key is nill")
			}
			fmt.Println(data1)

			OutPutFile.WriteString(data1)
		}
			...
			for key, value := range parseKeyValuePairs(data) { // data is gone through and key value variables entered
				...
				if strings.Contains(args[0], ".") && (strings.Contains(key, args[0])) {
					...
					if strings.Contains(key, args[0]) || strings.Contains(value, args[0]) {
						...
						OutPutFile.WriteString(fmt.Sprintf("Server IP:\n%s\n%s:%s\n\n", serverIP, serverAtr, value))
		...
		// 	line = line + "\"EtcdValue\"" + ":" + "\"vahanapp10\"}"
		...
		// 		fmt.Printf("Key: %s is metered as %s succesfully", "servers/Physical/10.246.40.139/Hostname", "vahanapp01") //hardcoded version
		// 	}
		// 	/////
		// }

...
func listAll(w http.ResponseWriter, r *http.Request) {
	...
	for _, kv := range response.Kvs {
		...
		// Retrieve and display revision values for the current key
		revisions := []int64{kv.ModRevision}
		revisionValues, err := getRevisionValues(etcdClient, etcdKey, revisions)
		if err != nil {
			log.Printf("Failed to retrieve revision values: %v", err)
			http.Error(w, "Internal Server Error", http.StatusInternalServerError)
			return
		}

		for _, value := range revisionValues {
			fmt.Fprintf(w, "Revision Value: %s\n", value)
		}
